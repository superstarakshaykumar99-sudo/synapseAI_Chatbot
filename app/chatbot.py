...
def generate_response_groq(session_id: str, query: str) -> Iterator[str]:
    if not groq_client:
    ...
    logger.info(f"Sending query to Groq ({GROQ_MODEL})...")
    try:
...
def generate_response(session_id: str, query: str) -> Iterator[str]:
    ...
    if AI_PROVIDER == "openai":
        ...
        yield from generate_response_groq(session_id, query)
    else:
